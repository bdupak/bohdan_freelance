...
public class OrderingService extends GenericService<Ordering, Integer> {
    ...
    public OrderingService(GenericDao genericDao) {
        super(genericDao);
    }
    ...
    private Map<ValidationParametersBuilder.Parameters, String> prepareData(Map<String, String[]> data) {
        ...
        map.put(ValidationParametersBuilder.createParameters(false).minLength(5)
                        .maxLength(120),
                data.get("title") == null ? null : data.get("title")[0]);
        map.put(ValidationParametersBuilder.createParameters(false).pattern("(hourly)|(fixed)"),
        ...
        map.put(ValidationParametersBuilder.createParameters(true).isInteger(true).min(1.00),
                data.get("customer_id") == null ? null : data.get("customer_id")[0]);
        map.put(ValidationParametersBuilder.createParameters(true).isInteger(false).min(0.00),
                data.get("payment") == null ? null : data.get("payment")[0]);
